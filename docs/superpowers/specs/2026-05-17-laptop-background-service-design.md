...
| `~/.config/whatsapp-deleter/targets.txt` | Per-user config: one phone number per line, `#` comments. Pre-seeded with `972523905680`. |
...
Pre-seeded with `972523905680` and a header comment explaining format (international, no `+`, no leading 0; substring-matched against `contact.number`).
